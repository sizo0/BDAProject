...
public class XQueryToXQueryX implements XPathTreeConstants {
	...
	private void xqprintEscaped(String s) {
		xqprintEscaped(s, false);
	}

	private void xqprintEscaped(String s, boolean escapeAmp) {
	...
	public boolean transform(SimpleNode node) {
		...
		case JJTSINGLETYPE: {
			...
			if (sn(node.jjtGetChild(0)).m_value != null) {
		...
		case JJTENDTAGQNAME:
			...
			if (!openTag.getValue().equals(node.getValue()))
				throw new PostParseException(
						"Error: In a direct element constructor, the name used in the end tag must exactly match the name used in the corresponding start tag, including its prefix or absence of a prefix.");
			return check(node);
		...
		case JJTTAGQNAME: {
			...
			String qname = null;
			if (!node.getValue().equals("xmlns")) {
				qname = processPrefix(node.m_value);
			}
			flushOpen(node, false);
			if (!node.getValue().equals("xmlns") && qname != null) {
				xqprint(qname);
			}
		...
		case JJTVARNAME: {
			...
			int pid = parent.id;
			if (/* pid == JJTCASECLAUSE || */pid == JJTTYPESWITCHEXPR) {
				...
				for (int i = 0; i < node.jjtGetNumChildren(); i++) {
					...
					if (isFilter) {
						...
						if (child.id == JJTPARENTHESIZEDEXPR) {
							pushElem("xqx:sequenceExpr", node);
							flushOpen(node, true);
							// if(getFirstChildOfFirstChildID(child) ==
							// JJTSTEPEXPR){
							// pushElem(id, node);
							// flushOpen(node, true);
							// }
			...
			if (isFilter) {
				...
				if (childID == JJTFORWARDAXIS || childID == JJTREVERSEAXIS) {
					...
					if (optionalAttribIndicator != null
						...
						// if (prev != null && (prev.id == JJTSLASHSLASH ||
						// true))
						{
							pushElem("xqx:xpathAxis", node);
							flushOpen(node, false);
		...
		case JJTCOMMENTCONTENTCHAR:
			xqprintEscaped(node.m_value, true);
			break;
